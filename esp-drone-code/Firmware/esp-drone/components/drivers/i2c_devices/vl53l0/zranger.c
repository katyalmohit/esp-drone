...
{
  // DEBUG_PRINTI("Sab OKAY hai\n");
  // DEBUG_PRINTI("variable isInit=%d\n",isInit);
  if (isInit)
    return;
  ...
  DEBUG_PRINTI( "I2C dev0 initialization");

  i2cdevInit(dev.I2Cx);
  DEBUG_PRINTI( "I2C dev1 initialization");

  // dev.io_timeout = 0;
  dev.io_timeout = 33000;
  dev.did_timeout = 0;
  ...
  // DEBUG_PRINTI( "I2C dev1 initialization");
  vTaskDelay(M2T(20));
  // DEBUG_PRINTI( "I2C dev2 initialization");

  uint16_t wordData;
  // DEBUG_PRINTI( "I2C dev3 initialization");
